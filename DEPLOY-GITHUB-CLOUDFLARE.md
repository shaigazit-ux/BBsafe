...
## Add live replacement data

After the first deploy, go to Cloudflare Pages > your project > Settings > Environment variables and add `LIVE_STOPS_URL` pointing to a public JSON file. The new `/api/stops` endpoint will merge that file into the base dataset by `id`, so a fallback like `latrun-fallback` can turn into a verified stop without editing the frontend.

Recommended pattern:
- Keep the app in one GitHub repo.
- Keep live shelter overrides in a second tiny public repo or gist.
- Update only that JSON file when you verify a new shelter.
